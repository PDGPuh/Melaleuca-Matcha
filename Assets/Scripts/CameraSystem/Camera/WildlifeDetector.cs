...
namespace RungTramTraSu.CameraSystem
{
    public class WildlifeDetector : MonoBehaviour
    {
        ...
        private void ProcessTargetCandidate(GameObject go, List<DetectedTarget> list)
        {
            ...
            {
                // Check bottom of bounds
                Vector3 bottomPoint = col.bounds.center - new Vector3(0f, col.bounds.extents.y, 0f);
                if (IsInViewfinder(bottomPoint, out viewportPos, 0.1f))
                {
                    inView = true;
                }
                else
                {
                    // Check top of bounds
                    Vector3 topPoint = col.bounds.center + new Vector3(0f, col.bounds.extents.y, 0f);
                    if (IsInViewfinder(topPoint, out viewportPos, 0.1f))
                    {
                        inView = true;
                    }
                }
        ...
        private void PopulateTargetMetadata(GameObject go, ref DetectedTarget target)
        {
            ...
            if (animal != null)
            {
                ...
                target.isRare = (animal.Type == AnimalAI.AnimalType.Stork); 
                return;
            }
